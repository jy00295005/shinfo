<nav class="nav nav-fill flex-column" id="side_link">

    <li class="nav-item active col-12 p-0">
        <a href="{{URL('test/research_projects')}}" class="nav-item btn btn-primary btn-block">科研项目情况</a>
    </li>

    <li class="nav-item col-12 p-0">
        <a href="{{URL('test/funding_group')}}" class="nav-item btn btn-block">资助机构及强度</a>
    </li>

    <li class="nav-item col-12 p-0">
        <a class="nav-item btn btn-block" href="{{URL('test/funding_analyzing')}}">资助主题分布</a>
    </li>

    <li class="nav-item col-12 p-0">
        <a class="nav-item btn btn-block disabled" href="{{URL('test')}}">各主题项目概况</a>
    </li>

    <li class="nav-item col-12 p-0">
        <a class="nav-item btn btn-block disabled" href="{{URL('test')}}">多项对比</a>
    </li>

</nav>